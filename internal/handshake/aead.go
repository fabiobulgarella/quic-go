...
func (s *sealer) EncryptHeader(sample []byte, firstByte *byte, pnBytes []byte) {
	...
	if s.is1RTT {
		*firstByte ^= s.hpMask[0] & 0x7 // changed from 0x1f so DelaySample and VEC are not encrypted
	} else {
...
func (o *opener) DecryptHeader(sample []byte, firstByte *byte, pnBytes []byte) {
	...
	if o.is1RTT {
		*firstByte ^= o.hpMask[0] & 0x7 // changed from 0x1f because DelaySample bit is not encrypted
	} else {
